...
<div class="w-[100px] ml-[350px] mt-10">
    <div class="container w-[1145px]">
        <div class="container w-3/4 mx-auto mt-20">
            <div class="text-3xl font-bold mb-10">Tambah Data</div>
            <form action="{{ route('mobil.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div>
                    <div>
                        <label for="" class="text-base text-slate-900 font-semibold">Foto Mobil</label>
                        <input class="w-full bg-slate-200 py-2 px-4" id="image" type="file" name="image" required>
                    </div>
                    <div>
                        <label for="" class="text-base text-slate-900 font-semibold">Nama Mobil</label>
                        <input class="w-full bg-slate-200 py-2 px-4" id="name" type="text" name="name" required>
                    </div>
                    <div>
                        <label for="" class="text-base text-slate-900 font-semibold">Harga Sewa</label>
                        <input class="w-full bg-slate-200 py-2 px-4" id="price" type="text" name="price" required>
                    </div>
                </div>
